...
int mat_mat_mul(stack *my_stack){
	...
	if (a_cols != b_rows){
		fprintf(stderr, "error: '@' shape mismatch: a[%d %d] b[%d %d] (a.cols != b.rows)\n",
	...
	if (b_T == NULL) { instance_free(a); instance_free(b); return TF_ERR_MEM; }
	transpose(b->data, b_T, b_rows, b_cols);

	float *new_data = calloc((size_t)(a_rows * b_cols), sizeof(float));
	if (new_data == NULL) { free(b_T); instance_free(a); instance_free(b); return TF_ERR_MEM; }
...
int sum_arr (stack *my_stack){
	...
	if (arr == NULL) return TF_ERR_STACK;
...
int dot_product (stack *my_stack){
	...
	if (a->shape.row > 1) {
		fprintf(stderr, "error: '.': first argument is not a row vector\n");
		instance_free(a);
		instance_free(b);
		return TF_ERR_ARG;
	}
	if (b->shape.row > 1) {
		fprintf(stderr, "error: '.': second argument is not a row vector\n");
		instance_free(a);
		instance_free(b);
		return TF_ERR_ARG;
	}
	if (a->shape.col != b->shape.col) {
		fprintf(stderr, "error: '.': length mismatch [%d] != [%d]\n", a->shape.col, b->shape.col);
		instance_free(a);
		instance_free(b);
		return TF_ERR_SHAPE;
	}
	int n = a->shape.col;
	float *sum = malloc(sizeof(float));
	if (sum == NULL) { instance_free(a); instance_free(b); return TF_ERR_MEM; }
	float dot = 0.0f;
